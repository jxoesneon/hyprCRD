...
void LibEIHandler::handle_keyboard_event(struct ei_event* event) {
    ...
    if (type == EI_EVENT_KEYBOARD_KEY) {
        ...
        std::cout << "EI: Keyboard " << (is_press ? "press" : "release") << " keycode=" << keycode
                  << std::endl;
...
void LibEIHandler::handle_pointer_event(struct ei_event* event) {
    ...
    switch (type) {
        ...
        case EI_EVENT_POINTER_MOTION_ABSOLUTE: {
            ...
            pointer->send_motion_absolute(time, static_cast<uint32_t>(x), static_cast<uint32_t>(y),
        ...
        case EI_EVENT_BUTTON_BUTTON: {
            ...
            std::cout << "EI: Button " << (is_press ? "press" : "release") << " button=" << button
                      << std::endl;

...
bool Portal::init(LibEIHandler* handler) {
    ...
    std::cout << "[portal] Detected monitor: " << current_monitor.name << " ("
              ...
              << ", scale=" << current_monitor.scale << ") at (" << current_monitor.x << ","
              << current_monitor.y << ")" << std::endl;
        ...
        object
            ->addVTable(
                sdbus::registerMethod("CreateSession")
                    .implementedAs([this](sdbus::ObjectPath req, sdbus::ObjectPath sess,
                                          std::string app,
                ...
                sdbus::registerMethod("Start").implementedAs(
                    [this](sdbus::ObjectPath req, sdbus::ObjectPath sess, std::string app,
                           std::string win, std::map<std::string, sdbus::Variant> opts) {
                        return Start(req, sess, app, win, opts);
                    }),
                sdbus::registerMethod("NotifyPointerMotion")
                    .implementedAs([this](sdbus::ObjectPath sess,
                                          std::map<std::string, sdbus::Variant> opts, double dx,
                                          double dy) { NotifyPointerMotion(sess, opts, dx, dy); }),
                sdbus::registerMethod("NotifyPointerButton")
                    .implementedAs(
                        [this](sdbus::ObjectPath sess, std::map<std::string, sdbus::Variant> opts,
                               int32_t btn,
                               uint32_t st) { NotifyPointerButton(sess, opts, btn, st); }),
                sdbus::registerMethod("NotifyKeyboardKeycode")
                    .implementedAs(
                        [this](sdbus::ObjectPath sess, std::map<std::string, sdbus::Variant> opts,
                               int32_t key,
                               uint32_t st) { NotifyKeyboardKeycode(sess, opts, key, st); }),
                sdbus::registerMethod("NotifyPointerAxis")
                    .implementedAs([this](sdbus::ObjectPath sess,
                                          std::map<std::string, sdbus::Variant> opts, double dx,
                                          double dy) { NotifyPointerAxis(sess, opts, dx, dy); }),
                sdbus::registerMethod("ConnectToEIS")
                    .implementedAs([this](sdbus::ObjectPath sess, std::string app,
                                          std::map<std::string, sdbus::Variant> opts) {
...
std::tuple<uint32_t, std::map<std::string, sdbus::Variant>> Portal::CreateSession(
    sdbus::ObjectPath request_handle, sdbus::ObjectPath session_handle, std::string app_id,
    std::map<std::string, sdbus::Variant> options) {
    std::cout << "RemoteDesktop CreateSession called for " << app_id
              << " (session: " << session_handle << ")" << std::endl;
    ...
    try {
        ...
        sess_obj
            ->addVTable(
                sdbus::registerMethod("Close").implementedAs([this, session_handle]() {
                    ...
                    try {
                        auto hypr_sess = sdbus::createProxy(
                            *connection,
                            sdbus::ServiceName{"org.freedesktop.impl.portal.desktop.hyprland"},
                            session_handle);
                        hypr_sess->callMethod("Close").onInterface(
        ...
        std::cerr << "Warning: Could not export session object at " << session_handle << ": "
                  << e.what() << std::endl;
    }

    // Also register paired session in xdg-desktop-portal-hyprland so SelectSources succeeds
    try {
        auto hypr_proxy = sdbus::createProxy(
            *connection, sdbus::ServiceName{"org.freedesktop.impl.portal.desktop.hyprland"},
        ...
        std::cerr << "Warning: Failed to create paired ScreenCast session on hyprland: " << e.what()
                  << std::endl;
    }
...
std::tuple<uint32_t, std::map<std::string, sdbus::Variant>> Portal::SelectDevices(
    ...
    std::map<std::string, sdbus::Variant> response;
    response["types"] =
        sdbus::Variant(static_cast<uint32_t>(7));  // keyboard | pointer | touchscreen
    return {0, response};
}

std::tuple<uint32_t, std::map<std::string, sdbus::Variant>> Portal::Start(
    sdbus::ObjectPath request_handle, sdbus::ObjectPath session_handle, std::string app_id,
    std::string parent_window, std::map<std::string, sdbus::Variant> options) {
    ...
    try {
        ...
        if (sc_results.find("streams") != sc_results.end()) {
            ...
            std::cout << "Propagated screencast streams to RemoteDesktop.Start response."
                      << std::endl;
        }
        if (resp_code != 0) {
...
void Portal::NotifyPointerMotion(sdbus::ObjectPath session_handle,
                                 std::map<std::string, sdbus::Variant> options, double dx,
                                 double dy) {
...
void Portal::NotifyPointerButton(sdbus::ObjectPath session_handle,
                                 std::map<std::string, sdbus::Variant> options, int32_t button,
                                 uint32_t state) {
    uint32_t time = static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
                                              std::chrono::steady_clock::now().time_since_epoch())
                                              .count());
...
void Portal::NotifyKeyboardKeycode(sdbus::ObjectPath session_handle,
                                   std::map<std::string, sdbus::Variant> options, int32_t keycode,
                                   uint32_t state) {
    uint32_t time = static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
                                              std::chrono::steady_clock::now().time_since_epoch())
                                              .count());
    ...
    if (libei_handler && libei_handler->keyboard) {
        ...
        libei_handler->keyboard->send_modifiers(modifier_state_depressed, modifier_state_latched,
                                                modifier_state_locked, modifier_state_group);
...
void Portal::NotifyPointerAxis(sdbus::ObjectPath session_handle,
                               std::map<std::string, sdbus::Variant> options, double dx,
                               double dy) {
    uint32_t time = static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
                                              std::chrono::steady_clock::now().time_since_epoch())
...
sdbus::UnixFd Portal::ConnectToEIS(sdbus::ObjectPath session_handle, std::string app_id,
                                   std::map<std::string, sdbus::Variant> options) {
    ...
    if (!libei_handler || !libei_handler->keyboard || !libei_handler->pointer) {
        throw sdbus::Error(sdbus::Error::Name{"org.freedesktop.portal.Error.Failed"},
                           "Virtual devices not available");
    }

    struct eis* eis_context = eis_new(nullptr);
    if (!eis_context) {
        throw sdbus::Error(sdbus::Error::Name{"org.freedesktop.portal.Error.Failed"},
                           "Failed to initialize EIS context");
    }
    ...
    if (rc != 0) {
        ...
        throw sdbus::Error(sdbus::Error::Name{"org.freedesktop.portal.Error.Failed"},
                           "Failed to setup EIS fd backend");
    }
    ...
    switch (type) {
        ...
        case EIS_EVENT_SEAT_BIND: {
            ...
            size_t keymap_size = 0;
            {
                ...
                if (xkb_ctx) {
                    ...
                    if (xkb_km) {
                        keymap_str_alloc =
                            xkb_keymap_get_as_string(xkb_km, XKB_KEYMAP_FORMAT_TEXT_V1);
                        if (keymap_str_alloc) {
                            keymap_size = strlen(keymap_str_alloc) + 1;  // include NUL
                            std::cout << "[portal] XKB keymap loaded (" << keymap_size << " bytes)"
                                      << std::endl;
                        } else {
            ...
            const char* keymap_to_write = keymap_str_alloc ? keymap_str_alloc : keymap_fallback;
            if (!keymap_str_alloc) {
            ...
            if (memfd >= 0) {
                if (write(memfd, keymap_to_write, keymap_size) == (ssize_t)keymap_size) {
                    ...
                    struct eis_keymap* keymap =
                        eis_device_new_keymap(keyboard, EIS_KEYMAP_TYPE_XKB, memfd, keymap_size);
                    if (keymap) {
                    ...
                    std::cerr << "[portal] write keymap to memfd failed: " << strerror(errno)
                              << std::endl;
                }
                close(memfd);
            } else {
            ...
            if (libei_handler && libei_handler->pointer) {
                uint32_t time =
                    static_cast<uint32_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
                                              std::chrono::steady_clock::now().time_since_epoch())
                                              .count());
                libei_handler->pointer->send_motion_absolute(
                    time, static_cast<uint32_t>(x), static_cast<uint32_t>(y), current_monitor.width,
        ...
        case EIS_EVENT_SCROLL_DELTA: {
            ...
            if (libei_handler && libei_handler->pointer) {
                ...
                if (dx != 0.0) {
                    libei_handler->pointer->send_axis(time, WL_POINTER_AXIS_HORIZONTAL_SCROLL,
                                                      dx * scale_factor);
                    libei_handler->pointer->send_axis_stop(time, WL_POINTER_AXIS_HORIZONTAL_SCROLL);
                }
                if (dy != 0.0) {
                    libei_handler->pointer->send_axis(time, WL_POINTER_AXIS_VERTICAL_SCROLL,
                                                      dy * scale_factor);
                    libei_handler->pointer->send_axis_stop(time, WL_POINTER_AXIS_VERTICAL_SCROLL);
                }
                libei_handler->pointer->send_frame();
        ...
        case EIS_EVENT_KEYBOARD_KEY: {
            ...
            if (libei_handler && libei_handler->keyboard) {
                ...
                libei_handler->keyboard->send_modifiers(
                    modifier_state_depressed, modifier_state_latched, modifier_state_locked,

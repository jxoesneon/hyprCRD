...
private:
    ...
    void NotifyPointerMotion(sdbus::ObjectPath session_handle,
                             std::map<std::string, sdbus::Variant> options, double dx, double dy);

    void NotifyPointerButton(sdbus::ObjectPath session_handle,
                             std::map<std::string, sdbus::Variant> options, int32_t button,
                             uint32_t state);

    void NotifyKeyboardKeycode(sdbus::ObjectPath session_handle,
                               std::map<std::string, sdbus::Variant> options, int32_t keycode,
                               uint32_t state);

    void NotifyPointerAxis(sdbus::ObjectPath session_handle,
                           std::map<std::string, sdbus::Variant> options, double dx, double dy);

    sdbus::UnixFd ConnectToEIS(sdbus::ObjectPath session_handle, std::string app_id,
                               std::map<std::string, sdbus::Variant> options);
